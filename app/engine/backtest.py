...
from app.engine.forecast import forecast_months
...
from app.engine.pipeline import prepare_forecasts
...
def _partner_forecasts(monthly: pd.DataFrame, eligible: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["qty"] = (
        pd.to_numeric(frame["qty"], errors="coerce").fillna(0.0).clip(lower=0.0)
    )
