...
| `docker-compose.yml` | **`db`**: `postgres:16-alpine`; `POSTGRES_DB=astrea`, `POSTGRES_USER=astrea`, `POSTGRES_PASSWORD=${POSTGRES_PASSWORD:?задайте в .env}`; том `pg_data:/var/lib/postgresql/data`; `ports: ["127.0.0.1:5432:5432"]` (для `pytest` с хоста; снаружи недоступен); healthcheck `pg_isready -U astrea`; `restart: unless-stopped`. **`app`**: `build: .`; `environment: DATABASE_URL=postgresql://astrea:${POSTGRES_PASSWORD}@db:5432/astrea`; `depends_on: db: condition: service_healthy`; `ports: ["127.0.0.1:8501:8501"]` (на VPS наружу не торчит — туда ходит Caddy); `restart: unless-stopped`. Том `pg_data`. |
